...
void UpCurrentData()
{
  ...
  {
    String Path = "Value";
    dienRead1 = pzem.energy()*1000;
     if (dienRead1 > SoDien_CB)
      {
        GUISMS(SDT, "Canh bao su dung dien qua muc!");
      }
//   float dienCall1 = dienRead1 - dienSet;
    Firebase.setFloat(fbdt, Path + "/Dien", dienRead1)?
    Serial.println("cập nhật số điện hiện tại xong") :
    Serial.println("Error");
     Serial.println(dienRead1);
...
void SendData() {
  if (millis() - timer1 > 60000) {
    if (modeDay == 0) {
      while (isnan(pzem.energy())) {}
      energy = ((pzem.power()/1000)/30)*5000;
//        energy = pzem.energy()*1000;
//      dienCal = dienRead-dienSet;
      dienMonth[ngay] = energy;
      nuocMonth[ngay] = luuLuong;
      Serial.println(dienRead);
      Serial.println(dienCal);
      Serial.println("Nuoc: " + (String)luuLuong + "---Dien: " + (String)energy);
      ...
      String parentPath = "DienNuoc/" + thang_Set + '-' + ngay_Set + '-' + (String)nam;
      Firebase.setFloat(fbdt, parentPath + "/Dien", energy) ?
...
void SendMonth() {
  if (ngay == 30) {
    if (modeMonth == 0) {
      String parentPath = "Total/" + (String)thang + '-' + (String)nam;
      for (int i = 1; i <= 30; i++) {
...
void S401Cal() {
  luuLuong = xung *1000/ 98.0 / 60.0;  // CT tính lưu lượng - đơn vị mL/s
